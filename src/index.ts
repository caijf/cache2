...
class Cache2<ValueType = any> extends Emitter<(key: string, value: ValueType) => void> {
  ...
  private _parse(value: any): CacheRecord<ValueType> {
    // 缓存在内存不需要转换
  ...
  private _stringify(value: CacheRecord<ValueType>) {
    // 缓存在内存不需要转换
    if (!this.options.needParsed) {
      return value;
    }
  ...
  private setCacheValues(values: CacheRecord<ValueType>) {
    this.options.storage.setItem(this.cacheKey, this._stringify(values) as any);
  }
